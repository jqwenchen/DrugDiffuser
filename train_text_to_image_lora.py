...
def main():
    ...
    if args.train_data_dir is not None:
        data_files["train"] = os.path.join(args.train_data_dir, f"{args.dataset_name}_data.csv")
    ...
    def preprocess_train(examples):
        image_path = [os.path.join(args.train_data_dir, args.dataset_name, f"{image}.jpg") for image in examples[image_column]]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # timesteps = torch.ones((bsz,), dtype=torch.long, device=latents.device) * noise_scheduler.config.num_train_timesteps
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        save_path = os.path.join(args.output_dir, f"checkpoint-{args.dataset_name}-{global_step}")
